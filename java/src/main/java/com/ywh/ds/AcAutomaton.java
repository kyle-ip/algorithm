...
public class AcAutomaton {
    ...
    public void insert(String word) {
        ...
        for (int i = 0; i < word.length(); i++) {
            ...
            if (cur.children[idx] == null) {
                cur.children[idx] = new AcNode(word.charAt(i));
            }
    ...
    private void buildFailurePointer() {
        //                              在 AC 自动机中，每条路径都视为一个模式串，每个节点都有一个失败指针。
        //                -             root 的失败指针指向 null。
        //             /  |  \
        //            a   b   c         沿 a b c 遍历到 c 时，最长可匹配后缀子串为 b c，
        //           /    |   ↑         其失败指针指向最长匹配后缀子串对应的模式串（另一条路径 b c d）的前缀（b c）的最后一个节点（c）。
        //          b +-> c---+
        //         /  |   |             可以逐层依次来求解每个节点的失败指针，失败指针的构建是一个按层遍历树的过程。
        //        c --+   d
        //       /        ↑
        //      d --------+
    ...
    public void match(String text) {
        ...
        // 如果 p 指向节点存在等于 b[i] 的子节点 x，则更新 p 指向 x。通过失败指针检测一系列失败指针为结尾的路径是否是模式串。
